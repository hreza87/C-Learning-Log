...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "3":
            ...
            for (int i = 0; i < maxPets; i++)
                {
                if (ourAnimals[i,0].StartsWith("ID #: ") && (ourAnimals[i,2].Substring(5) != "" && ourAnimals[i,2].Substring(2).Contains("?")))
                    {
                    do
                    {
                ...
                if (ourAnimals[i,4].Substring(22) == "" || ourAnimals[i,4].Substring(22).Contains("?"))
                    {
                    do
                     {
                        Console.WriteLine("Enter a physical description of the pet (size, color, gender, weight, housebroken)");
